...
class Executions:
    def __init__(self, path):
        ...
        self.df = self.df.rename(
            columns={1: 'ID', 2: 'Symb', 6: 'Code', 8: 'DateTime', 10: 'Shares', 12: 'Price', 13: 'Pos', 14: 'Comm'})
    ...
    def add(self, execution_id, symbol, code, date, shares, price, pos, comm):
        execution_data = {'ID': execution_id, 'Symb': symbol, 'Code': code, 'DateTime': date, 'Shares': shares,
                          'Price': price, 'Pos': pos, 'Comm': comm}
...
class Trades:
    def __init__(self):
        headers = ['Open', 'Close', 'Held', 'Symb', 'Side', 'Avr Entry', 'Avr Exit', 'Qty', 'Gross', 'Comm', 'Net',
                   'Open Qty', 'Status', 'Trade ID', 'Exec']
    ...
    def add(self, date, symbol, shares, trade_id):
        ...
        trade_data = {'Open': date, 'Symb': symbol, 'Open Qty': shares, 'Side': side, 'Status': 'Open',
                      'Trade ID': trade_id}
...
main_func()
print(trades.df[['Close', 'Symb', 'Side', 'Avr Entry', 'Avr Exit', 'Qty', 'Exec', 'Gross', 'Comm', 'Net',
                 'Status']].sort_values(by='Close', ascending=False))
# print(trades.df)
